...
const Layout = ({ children, projects, onSelectProject, onAddProject, onDeleteProject, selectedProjectId, currentView, onSelectView }) => {
    ...
    return (
        <div className="flex h-screen bg-gray-100 dark:bg-gray-900 transition-colors duration-200 overflow-hidden">
            {/* Mobile Header */}
            <div className="md:hidden fixed top-0 left-0 right-0 h-16 bg-white dark:bg-gray-800 border-b dark:border-gray-700 flex items-center justify-between px-4 z-20">
            ...
            {/* Main Content */}
            <div className="flex-1 overflow-auto p-4 md:p-8 text-gray-900 dark:text-gray-100 pt-20 md:pt-8">
                {children}
            </div>
        </div>
    );
